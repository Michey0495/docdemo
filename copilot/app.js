...
// ---- chat panel（Claudeとの会話画面を再現）----
function renderTerminal(execution) {
  const tool = execution.tool || 'GitHub Copilot Chat';
  ...
  const initHint = h('span', { class: 'terminal-line', style: 'color:#777' }, '右上「送信」ボタンを押すと Copilot の応答が流れます\n');
...
function renderPhase(phase, index) {
  ...
  const tabs = [
    ...
    { id: 'cfg', label: 'GitHub Copilot 設定', render: () => renderConfigPanel(phase) },
...
function renderSummary() {
  const stats = [
    { num: '10', label: '工程' },
    { num: '9',  label: 'チャットモード' },
    { num: '5',  label: 'Instructions' },
    { num: '5+', label: 'Actions ワークフロー' },
    { num: '4',  label: 'MCPサーバー' },
    { num: '5',  label: 'プロンプトファイル' },
  ];
