...
Claude Code はセッション開始時にこのファイルを読み込む。要求抽出から保守運用までの共通規約をここに集約し、工程固有の手順は .claude/agents/（専門サブエージェント）と .claude/commands/（パイプライン）に分離する。
...
- 技術選定を変更する場合は必ず ADR を design/basic/adr/ に追加する（.claude/templates/adr.md）
...
- 要件定義書の章立ては ISO/IEC/IEEE 29148 準拠（.claude/skills/rdd-template）
- 図は Mermaid に統一（PlantUML・画像貼り付けは不可。差分レビューできる形式のみ）
- 用語は要件定義書末尾の用語集を参照し、表記揺れを禁止
- 受身形を避け能動態で記述する
- すべての要件に FR/NFR/CON ID を付与し、章末に「関連要求ID一覧」を必ず付ける
- ドラフト生成後は req-reviewer サブエージェントが自動レビュー。WARN/ERROR が残ったままの commit は PreToolUse Hook が拒否する
...
- エラーコードは E_<ドメイン>_<連番3桁>（.claude/templates/error-code.md）。ユーザー向けとログ向けのメッセージを分ける
...
- commit 前に typecheck / lint が通っていること（PostToolUse Hook が自動実行）
- 危険コマンド（push --force / reset --hard / rm -rf / sudo）は Hook で遮断済み。必要な場合は人間が実行する
- PR には関連する US-ID を必ず記載する

## レビューフロー

- 生成物は必ず別のサブエージェントでレビューする（生成した本人によるセルフレビュー禁止）
- 実装PRは /full-review で security-review / code-reviewer / qa-engineer を並列実行
