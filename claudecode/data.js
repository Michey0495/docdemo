...
01:31:55 田中物流部長: 棚卸しは年4回。差異が出たら原因を追えるようにしたい。`,
    ...
    configFiles: [
      {
        kind: 'knowledge',
        path: 'CLAUDE.md（要求工学のプロジェクト規約 抜粋）',
        lang: 'md',
        body:
`# プロジェクト規約 - 要求工学
...
> /init で雛形生成、/memory でメモリ管理（いずれも公式コマンド）`,
      ...
      {
        kind: 'subagent',
        path: '.claude/agents/req-extractor.md',
        lang: 'md',
        body:
`---
name: req-extractor
...
- 入力20,000字超の場合は5,000字単位で分割処理しコンテキストを節約`,
      ...
      {
        kind: 'subagent',
        path: '.claude/agents/req-reviewer.md',
        lang: 'md',
        body:
`---
name: req-reviewer
...
出力: [INFO]/[WARN]/[ERROR] + 該当ID + 提案`,
      ...
      {
        kind: 'command',
        path: '.claude/commands/extract-requirements.md',
        lang: 'md',
        body:
`---
...
3. WARN/ERROR があれば次工程に進まず、人間に提示して確認`,
    ...
    officialRefs: [
      { label: '/agents（公式コマンド）', body: 'Claude Code 内で対話的にサブエージェントを作成・編集。frontmatter (name/description/tools/model) はこのコマンドが標準化。' },
      { label: '/memory（公式コマンド）', body: 'CLAUDE.md（プロジェクト/ユーザー/ローカル）を一元管理。要求工学の社内規約はここに常駐。' },
...
出力: [INFO]/[WARN]/[ERROR] + 該当ID + 提案`,
  ...
  // ───────────────────────── 02 ─────────────────────────
  {
    ...
    configFiles: [
      {
        kind: 'subagent',
        path: '.claude/agents/prioritizer.md',
        lang: 'md',
        body:
`---
name: prioritizer
...
- docs/02-stakeholder-review.md（先方確認シート）`,
      ...
      {
        kind: 'harness',
        path: '.claude/settings.json（権限の追加）',
        lang: 'json',
        body:
`{
  "permissions": {
    ...
    bestPractices: [
      { title: '迷ったらAskUserQuestion', body: '「Claude が勝手に決めた」を防ぐ。優先度はビジネス判断、技術判断ではない。' },
    ...
    officialRefs: [
      { label: 'AskUserQuestion ツール（公式）', body: 'Claude Code 標準ツール。複数選択肢を提示してユーザーの選択を待つ。サブエージェントの tools フィールドで明示的に許可。' },
    ...
    execution: {
      ...
      lines: [
        ...
        '  人間判断要請: 9件',
        '',
        '? AskUserQuestion 「FR-021 IoTセンサー連携」は今回のMustですか？',
        '  発言者の確信度: 低 / 予算逼迫: 該当',
...
佐藤情シス / 連携 / 確認依頼:
  ...
  // ───────────────────────── 03 ─────────────────────────
  {
    ...
    flow: {
      ...
      config:    { label: 'rdd-template skill + CLAUDE.md', detail: '社内規約テンプレ' },
      output:    { label: 'requirements-definition.md',    detail: '要件定義書 v1.0' },
    },
    configFiles: [
      {
        kind: 'knowledge',
        path: 'CLAUDE.md（ドキュメント規約）',
        lang: 'md',
        body:
`# プロジェクト規約 - ドキュメント
...
- WARN/ERRORが残ったままcommitしない（PreToolUse Hookで防止）`,
      ...
      {
        kind: 'skill',
        path: '.claude/skills/rdd-template/SKILL.md',
        lang: 'md',
        body:
`---
name: rdd-template
...
- ./templates/glossary.md`,
      ...
      {
        kind: 'hook',
        path: '.claude/settings.json（PreToolUse: WARN残りcommit禁止）',
        lang: 'json',
        body:
`{
  "hooks": {
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "if echo \\"$CLAUDE_TOOL_INPUT\\" | grep -q 'git commit' && grep -rE '\\\\[(WARN|ERROR)\\\\]' docs/ >/dev/null 2>&1; then echo 'WARN/ERRORが残っています'; exit 2; fi"
          }
        ]
      }
    ]
    ...
    bestPractices: [
      { title: '章構成は規約に強制させる', body: '「次の案件でも同じ品質で」を実現するには、人の記憶ではなくスキルに記憶させる。/skills で雛形を一元化。' },
    ...
    officialRefs: [
      { label: '/skills（公式コマンド）', body: 'Skill の作成・編集・有効化UIを起動。SKILL.md は frontmatter + 本文のフォーマット。' },
      { label: 'Hooks: PreToolUse（公式イベント）', body: 'ツール実行直前に検証スクリプトを差し込める。exit 2 で実行を中断、stderr の内容がClaudeへフィードバックされる。' },
...
（以下20章まで継続）`,
    ...
    review: {
      ...
      prompt: 'RDDの章立てがCLAUDE.md準拠か、要求IDのトレーサビリティが全項目で取れているか、図表ルール（Mermaid限定）が守られているかチェック。',
      comments: [
...
\`\`\``,
    ...
    officialRefs: [
      { label: '/agents', body: 'spec-writer の tools/model フィールドはこのコマンドで対話的に設定。' },
...
tools: Read, Write, WebSearch, mcp__github__*
model: opus
---
...
`{
  "mcpServers": {
    ...
    officialRefs: [
      { label: 'GitHub MCP（GitHub公式提供）', body: 'PR/Issue/コメントをClaude Codeから直接操作。設計決定をPRとして残せる。' },
      { label: '/mcp（Claude Code公式コマンド）', body: 'MCPサーバーの追加・削除・状態確認UI。.mcp.json（プロジェクト共通）と ~/.claude.json（ユーザー固有）を区別管理。' },
...
- MySQL: JSONB対応に難 → 却下`,
  ...
  // ───────────────────────── 06 ─────────────────────────
  {
    ...
    configFiles: [
      {
        kind: 'subagent',
        path: '.claude/agents/detail-designer.md',
        lang: 'md',
        body:
`---
...
- エラーコード一覧 (機能ドメインプレフィクス: E_AUTH_xxx, E_STOCK_xxx)`,
      ...
      {
        kind: 'knowledge',
        path: '.claude/templates/error-code.md',
        lang: 'md',
        body:
`# エラーコード命名規約
...
| 入出庫 | 数量 | ○ | int | 1 - 99999 | E_STOCK_022 | 数量は1-99999で入力してください |`,
  ...
  // ───────────────────────── 07 ─────────────────────────
  {
    ...
    flow: {
      ...
      config:    { label: 'settings.json + 5種Hooks', detail: 'PreTool/PostTool/Stop' },
      output:    { label: 'PR diff',                detail: 'GitHub Pull Request' },
    },
    configFiles: [
      {
        kind: 'knowledge',
        path: 'CLAUDE.md（コーディング規約）',
        lang: 'md',
        body:
`# コーディング規約
...
`{
  ...
  "env": {
    "NODE_ENV": "development"
      ...
      {
        kind: 'hook',
        path: '.claude/settings.json（Hooks 抜粋）',
        lang: 'json',
        body:
`{
  "hooks": {
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "echo '[Context] branch=\$(git branch --show-current) story=\${STORY_ID:-unset}'"
          }
        ]
      }
    ]
...
exit 0`,
      ...
      {
        kind: 'hook',
        path: '.claude/hooks/check-secrets.sh',
        lang: 'bash',
        body:
`#!/usr/bin/env bash
...
- レビューだけ欲しい場合は /simplify --review-only`,
      ...
      {
        kind: 'command',
        path: '.claude/commands/implement-feature.md',
        lang: 'md',
        body:
`---
...
- mutateしない（CLAUDE.md 規約）`,
    ...
    bestPractices: [
      { title: 'ガードレールはHooksで敷く', body: 'PostToolUse(Edit|Write)で typecheck/lint を自動。「忘れる人間」を補強。' },
      { title: '危険コマンドはdenyではなくPreToolUseで完全遮断', body: 'permissions.deny は確認プロンプトで止まるが回避され得る。Hookの exit 2 は物理的に阻止。' },
      { title: '${CLAUDE_PROJECT_DIR}を使う', body: '相対パスはサブシェルで壊れる。公式の環境変数を使うと堅牢。' },
      { title: 'Stop Hookでテストを最終ガード', body: 'Claude が「完了」と返す前にテストが落ちていないかをHookで検査。' },
      { title: 'includeCoAuthoredBy=trueでCo-Author自動付与', body: '人間と同じ責任構造でcommit。後から「誰の判断だったか」を追える。' },
      { title: '1ストーリー=1ブランチ=1PR', body: 'PRサイズが小さいほどレビュー所要時間は二乗で短くなる。' },
    ],
    officialRefs: [
      { label: 'Anthropic公式ドキュメント: Hooks', body: '8イベント (PreToolUse/PostToolUse/UserPromptSubmit/Notification/Stop/SubagentStop/PreCompact/SessionStart/SessionEnd) の入出力JSONスキーマと exit code セマンティクス。' },
      { label: '${CLAUDE_PROJECT_DIR}（公式環境変数）', body: 'Hooks 内でプロジェクトルート絶対パスを取得。sub-shell 配下でも壊れない。' },
      { label: '/hooks（公式コマンド）', body: 'Hooks の追加・編集・有効/無効切り替えUI。settings.json直編集と同等の効果。' },
      { label: '/permissions（公式コマンド）', body: 'permissions.allow/deny を対話的に追加。Bash パターンマッチは "Bash(cmd:*)" 構文。' },
...
`---
name: code-reviewer
description: PRのdiffを精査。CLAUDE.md規約・命名・不変性・例外境界・性能を確認
tools: Read, Bash, mcp__github__get_pull_request, mcp__github__create_pr_comment
model: opus
---
レビュー観点:
1. CLAUDE.md規約準拠
...
`{
  "mcpServers": {
    "playwright": {
      ...
      {
        kind: 'command',
        path: '.claude/commands/full-review.md',
        lang: 'md',
        body:
`---
...
3者の出力を docs/08-review-report.md に統合し、自動修正可能な指摘は適用、人判断が必要なものは残置。`,
    ...
    bestPractices: [
      { title: '3エージェント並列で時間圧縮', body: 'security / code / qa を直列に回すと2-3時間。並列なら45分。Claude Code はAgent並列実行を標準でサポート。' },
    ...
    officialRefs: [
      { label: 'security-review skill（Anthropic公式提供パターン）', body: 'Claude Code 公式が紹介する代表的Skill。OWASP Top10ベースで diff を精査。' },
...
`{
  "mcpServers": {
    "github": {
      ...
      {
        kind: 'hook',
        path: '.claude/settings.json（Stop Hook で本番直前チェック）',
        lang: 'json',
        body:
`{
  "hooks": {
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "if git diff --name-only HEAD~1 | grep -q '^infra/'; then echo 'IaC変更検知 - terraform plan を確認してください'; fi"
          }
        ]
      }
    ]
...
resource "aws_vpc" "sftp_bridge" {
  ...
  // ───────────────────────── 10 ─────────────────────────
  {
    ...
    flow: {
      ...
      operation: { label: 'cron-scheduled agent',  detail: 'CronCreate定期実行' },
    ...
    configFiles: [
      {
        kind: 'harness',
        path: '.claude/triggers/daily-health-check.json',
        lang: 'json',
        body:
`{
  ...
  "cron": "0 9 * * *",
  "timezone": "Asia/Tokyo",
  "prompt": "/health-check 24h を実行し、エラー率/レスポンス分布/CSV連携成否をDatadog MCP経由で取得し、reports/daily/YYYY-MM-DD.md にまとめる。閾値超えがあれば Slack に通知。",
  "tools": ["mcp__datadog__*", "mcp__slack__*", "Write"]
}`,
      },
      {
        kind: 'skill',
        path: '.claude/skills/postmortem/SKILL.md',
        lang: 'md',
        body:
`---
name: postmortem
...
- 必ず「仕組み」「自動化」「Hook化」で書く`,
      ...
      {
        kind: 'mcp',
        path: '.mcp.json（Datadog / Slack MCP）',
        lang: 'json',
        body:
`{
  "mcpServers": {
...
tools: Read, Write, Bash, mcp__datadog__*, mcp__slack__*
model: opus
---
動作:
...
5. Slack #incident に共有、対応方針を募る`,
      ...
      {
        kind: 'command',
        path: '.claude/commands/incident-respond.md',
        lang: 'md',
        body:
`---
...
postmortem skill を適用し、ドラフトを incidents/\$ARGUMENTS/postmortem.md に書き出す。`,
    ...
    officialRefs: [
      { label: 'Anthropic公式: Triggers / Schedule', body: 'CronCreate / CronList / CronDelete でリモートエージェントを cron 形式で起動。' },
      { label: 'Slack MCP（公式提供）', body: '@modelcontextprotocol/server-slack。チャネル投稿・スレッド返信・ユーザー検索が標準操作。' },
      { label: 'Datadog MCP（Datadog公式）', body: 'メトリクス/ログ/モニタを Claude Code から直接取得。インシデント対応で必須。' },
    ...
    execution: {
      ...
      lines: [
        '> 定期トリガーを登録...',
        '  cron: 0 9 * * * (毎朝9時 JST)',
        '  ツール権限: Datadog / Slack / Write',
        '✓ Trigger ID: trg_8a3f9c',
        '',
        '--- 翌朝9時 自動実行 ---',
        '> /health-check 24h',
