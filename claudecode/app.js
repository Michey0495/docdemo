...
function renderTerminal(execution) {
  const tool = execution.tool || 'Claude Code';
  ...
  const initHint = h('span', { class: 'terminal-line', style: 'color:#777' }, '右上「送信」ボタンを押すと Claude の応答が流れます\n');
...
function renderPhase(phase, index) {
  ...
  const tabs = [
    ...
    { id: 'cfg', label: 'Claude Code 設定', render: () => renderConfigPanel(phase) },
