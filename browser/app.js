// ============================================
// 23DocDemo: Claude Code 上流→下流 説明資料
// ============================================
...
// ---- chat panel（ブラウザAIチャットの会話再現）----
function renderTerminal(execution) {
  const body = h('div', { class: 'terminal-body' });
  const cmdLine = h('div', {}, [
    h('span', { class: 'terminal-prompt' }, '> ' + (execution.tool || 'ChatGPT') + ' に送信'),
    h('br'),
  ...
  return h('div', { class: 'terminal' }, [
    h('div', { class: 'terminal-head' }, [
      ...
      h('div', { class: 'terminal-title' }, execution.tool || 'ブラウザAIチャット'),
      runBtn,
    ]),
    body,
  ]);
...
const KIND_ORDER = ['instructions', 'workspace', 'prompt', 'attachment', 'connector', 'routine', 'review'];
...
function renderReview(review) {
  const items = [
    h('div', { class: 'section-label' }, 'レビュー手順'),
    h('div', { class: 'section-desc' }, review.skillName),
    renderFile({ name: 'レビュー依頼プロンプト', lang: 'md', body: review.prompt }),
    h('div', { class: 'spacer' }),
...
function renderSummary() {
  const stats = [
    { num: '10', label: '工程' },
    { num: '4',  label: '併用ツール' },
    { num: '12', label: 'Projects / Gem' },
    { num: '30+', label: 'テンプレートプロンプト' },
    { num: '5',  label: 'コネクタ / 拡張' },
    { num: '3',  label: '定期実行ルーチン' },
  ];
